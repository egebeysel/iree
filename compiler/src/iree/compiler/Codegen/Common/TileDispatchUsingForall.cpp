...
void TileAndDistributeToWorkgroupsUsingForallOpPass::runOnOperation() {
  ...
  // llvm/llvm-project#150185). At the distribution/workgroup level the loop tile
  // size is an integer MULTIPLE of the inner tile on scalable dims (the vector
  // tile equals the inner tile, the distribution tile is a multiple of it), so
  // each scalable dim is Multiple; static dims stay Unknown and are handled by
  // the existing static path. The control function is evaluated by the SCF
  // driver once per tiled/fused pack/unpack op, in that op's own iteration
  // domain (keying off its scalable, i.e. dynamic, inner tiles), so it threads
  // correctly through intervening ops such as a transposing producer; every
  // other op gets no hint.
